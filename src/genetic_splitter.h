...
class genetic_graph_splitter {
	...
	typedef EA::Genetic<genetic_graph_splitter::MySolution,
						genetic_graph_splitter::MyMiddleCost>
		GA_Type;
	typedef EA::GenerationType<genetic_graph_splitter::MySolution,
							   genetic_graph_splitter::MyMiddleCost>
		Generation_Type;
   ...
   public:
	...
	vector<vector<Node>> partition_graph_with_split(
		const vector<pair<Node, int>>& loc_split, double* score);
	vector<vector<Node>>& fix_solution(vector<vector<Node>>& solution);

   private:
	bool eval_solution(const genetic_graph_splitter::MySolution& p,
					   genetic_graph_splitter::MyMiddleCost& c);
	void init_genes(genetic_graph_splitter::MySolution& p,
					const std::function<double(void)>& rnd01);
	genetic_graph_splitter::MySolution mutate(
		const genetic_graph_splitter::MySolution& X_base,
		const std::function<double(void)>& rnd01, double shrink_scale);
	genetic_graph_splitter::MySolution crossover(
		const genetic_graph_splitter::MySolution& X1,
		const genetic_graph_splitter::MySolution& X2,
		const std::function<double(void)>& rnd01);
