...
bool genetic_optimizer::eval_solution(const MySolution& p, MyMiddleCost& c) {
	...
	if (zone_id < zones.size() - 1) {
		...
		for(Dijkstra<Graph>::Path::RevArcIt it(path); it!=INVALID ; it.operator++() ) {
			if (find(zones[zone_id + 1].begin(), zones[zone_id + 1].end(), navigator->graph.source(it)) != zones[zone_id + 1].end()) break;
			cost_to_next_zone++;
		}
	}
...
MySolution genetic_optimizer::mutate(const MySolution& X_base,
	...
	int nb_of_mutations = max(1, (int)(1. * rnd01() * shrink_scale));
	...
	for (int i = 0; i < nb_of_mutations; i++) {
		...
		if (action < .5) {
			...
			orientation new_orient =(orientation)((X_new.node_list[swap1].second  + (int)(4. * rnd01())) % 4);
			if (new_orient > WEST) {
				cout << "issue" << endl;
			}
			X_new.node_list.erase(X_new.node_list.begin() + swap1);
			X_new.node_list.emplace(X_new.node_list.begin() + swap1, node.first, new_orient);

			node = X_new.node_list[swap2];
			new_orient = (orientation)((X_new.node_list[swap2].second  + (int)(4. * rnd01())) % 4);
			if (new_orient > WEST) {
				cout << "issue" << endl;
			}
			X_new.node_list.erase(X_new.node_list.begin() + swap2);
			X_new.node_list.emplace(X_new.node_list.begin() + swap2, node.first, new_orient);
			//X_new.node_list[swap1].second = (orientation)(((int)X_new.node_list[swap1].second  + (int)(4. * rnd01())) % 4);
		}
	}
	return X_new;
}
...
MySolution genetic_optimizer::crossover(const MySolution& X1, const MySolution& X2,
    ...
    while (1) {
		...
		while (find(nodes_to_check.begin(), nodes_to_check.end(), node.first) != nodes_to_check.end() && i) {
            i--;
			node = X2.node_list[j++];
            if (j == (int)X1.node_list.size()) j = 0;
		}
...
void genetic_optimizer::SO_report_generation(
	...
	output_file << generation_number << "   \t" << std::setw(11)
				...
				<< best_genes.to_string(this).first << "\n";
	output_file.flush();
	output_file.close();*/
}

genetic_optimizer::genetic_optimizer(int arg_instance, mine_navigator *arg_nav, vector<vector<position>> &arg_zones, int arg_zone_id, Node start_node, string start_string):
	...
	cout << "input:" << start_string << " node: " << navigator->graph.id(start_node) << " coord: " << navigator->coord_map[start_node] << " orientation: " << base_mine.current_orientation << endl;
